...
/**
 * Defines an HTTP exception for *Range Not Satisfiable* type errors.
 */
export class RangeNotSatisfiableException extends HttpException {
  /**
   * Instantiate an `RangeNotSatisfiableExceptino` exception.
   *
   * @example
   * ```ts
   * throw new RangeNotSatisfiableExceptino();
   * ```
   *
   * @usageNotes
   * The HTTP response status wll be 416.
   ...
   * - `message`: the string `"Range Not Satisfiable"` by default; override this by supplying a string
  ...
  public constructor(
    ...
    const { description, httpExceptionOptions } = HttpException
      .extractDescriptionAndOptionsFrom(
        descriptionOrOptions ?? STATUS_TEXT[StatusCode.RangeNotSatisfiable],
      );
