...
export class UnauthorizedException extends HttpException {
   ...
   *
   * @usageNotes
   * The HTTP response status wll be 401.
   ...
   * - `message`: the string `"Unauthorized"` by default; override this by supplying a string
  ...
  public constructor(
    ...
    const { description, httpExceptionOptions } = HttpException
      .extractDescriptionAndOptionsFrom(
        descriptionOrOptions ?? STATUS_TEXT[StatusCode.Unauthorized],
      );
