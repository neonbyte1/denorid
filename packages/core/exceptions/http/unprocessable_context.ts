...
/**
 * Defines an HTTP exception for *Unprocessable Context* type errors.
 */
export class UnprocessableContentException extends HttpException {
  /**
   * Instantiate an `UnprocessableContentException` exception.
   *
   * @example
   * ```ts
   ...
   *
   * @usageNotes
   * The HTTP response status wll be 422.
   ...
   * - `message`: the string `"Unprocessable Content"` by default; override this by supplying a string
  ...
  public constructor(
    ...
    const { description, httpExceptionOptions } = HttpException
      .extractDescriptionAndOptionsFrom(
        descriptionOrOptions ?? STATUS_TEXT[StatusCode.UnprocessableContent],
      );
