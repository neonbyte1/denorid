...
describe(Server.name, () => {
  ...
  describe("registerHandlers", () => {
    it("registers handlers from decorated controller types", () => {
      ...
      server.registerHandlers([TestCtrl as unknown as Type], [instance]);
    ...
    it("skips types with no message mapping metadata", () => {
      class Plain {}
      server.registerHandlers([Plain as unknown as Type], [new Plain()]);
  ...
  describe("dispatch", () => {
    ...
    it("throws when the registered method is not a function", async () => {
      server["handlers"].set("bad", {
        instance: { method: "not-a-function" },
        methodName: "method",
        type: "message",
        controllerType: class {} as unknown as Type,
      });
    ...
    it("logs error when no exceptionHandler is set and handler throws", async () => {
      ...
      server.registerHandlers([ErrCtrl2 as unknown as Type], [new ErrCtrl2()]);
  ...
  describe("dispatch - guard enforcement", () => {
    ...
    it("allows dispatch when CanActivate guard returns true", async () => {
      ...
      server.registerHandlers([CtrlOk as unknown as Type], [new CtrlOk()]);
